# Testing Tungsten 2.0 on a laptop
...
Use `Tungsten-IDE-2.0.0-win-*.exe`:
...
```bash
chmod +x Tungsten-IDE-2.0.0-*.AppImage
./Tungsten-IDE-2.0.0-*.AppImage
```

Install the Debian package with:

```bash
sudo apt install ./Tungsten-IDE-2.0.0-*.deb
```

## Acceptance checklist

1. Launch Tungsten and open a local source-code folder.
2. Create, edit, save, rename, search, and delete a temporary file.
3. Open two terminal tabs, split the terminal, and run an interactive shell command.
...
6. Open Testing, refresh discovery, run one test, and load an LCOV report if the project provides one.
7. Open Source Control and test diff, stage, commit, history, blame, stash, and GitHub lists.
...
9. Host a collaboration room on one computer and join from another on the same trusted network. Verify shared text, presence, and line comments.
